...
namespace RichillCapital.SharedKernel.UnitTests.Specifications.Evaluators;
...
public class StringExtensionsTests
{
    ...
    public void Like_Should_ReturnsExpectedResult(
        bool expectedResult,
        string pattern,
        string input)
    {
    ...
    [InlineData("[", "asd")]
    [InlineData("[]", "asd")]
    public void Like_Should_ThrowInvalidSearchPattern(string pattern, string input)
    {
        Action action = () => input.Like(pattern);

        action.Should()
            .Throw<InvalidSearchPatternException>()
            .WithMessage($"Invalid search pattern: {pattern}");
    }
}